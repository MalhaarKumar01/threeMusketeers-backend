...
public class Board implements LearnerBoard{
    public int size = 5;
    ...
    public Board getCopyBoard(Board b) {
    	// Creates a copy board
    	Board brd = new Board(b);
    	brd.size = b.size;
    	for (int row = 0; row < size; row++) {
    		for (int col = 0; col < size; col++) {
    			brd.board[row][col] = new Cell(board[row][col]);
    		}
    	}
    	brd.turn = getTurn();
    	brd.winner = getWinner();
    	return brd;
    }
	...
	public void newRound() {
		// Adds the current round to the savedBoard of Three Musketeers
		assignment1.ThreeMusketeers.savedBoards.add(this);
		System.out.println("Added round to previous rounds");
		// Calls to restart the game and display the winning rounds
		assignment1.ThreeMusketeers.restartGame();
		assignment1.ThreeMusketeers.winningRounds();
	}
}